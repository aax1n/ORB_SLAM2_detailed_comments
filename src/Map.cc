 ...
 */
/// 设置参考地图点------用于绘图显示局部地图点（红色）
void Map::SetReferenceMapPoints(const vector<MapPoint *> &vpMPs)  ///传进来一个MapPoint * 类型的数组  用引用的方式
{
    unique_lock<mutex> lock(mMutexMap);
    mvpReferenceMapPoints = vpMPs;   ///mvpReferenceMapPoints 地图中红色的那些点
}
