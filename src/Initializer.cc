                             ...
                             vector<bool> &vbTriangulated)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
    {
		///vMatches12[i]解释：i表示帧1中关键点的索引值，vMatches12[i]的值为帧2的关键点索引值
        ///没有匹配关系的话，vMatches12[i]值为 -1
        if(vMatches12[i]>=0)   ///  > 0 ===> 说明有匹配关系
        {
			/// mvMatches12 中只记录（已经匹配上的吧？）有匹配关系的特征点对的索引值
            /// i表示帧1中关键点的索引值，vMatches12[i]的值为帧2的关键点索引值
            /// todo vMatches12  mvMatches12 区别 ： vMatches12 记录所有匹配关系  mvMatches12 记录有匹配关系的点对
    ...
    /// 新建一个容器vAllIndices存储特征点索引，并预分配空间  后面八点法用的
    ...
    /**
     * RANSAC算法的基本假设是样本中包含正确数据(inliers，可以被模型描述的数据),
    ...
    vector<bool> vbMatchesInliersH, vbMatchesInliersF;  ///记录当前点是不是有效的内点
	/// todo 算出来的单应矩阵和基础矩阵的RANSAC评分，这里其实是采用重投影误差来计算的
    /// score for H and F
    float SH, SF;
    /// 这两个是经过RANSAC算法后计算出来的单应矩阵和基础矩阵
    cv::Mat H, F; 

    /// 构造线程来计算H矩阵及其得分 开了两个线程
    ...
    if(RH>0.40)
        ...
        return ReconstructH(vbMatchesInliersH,	/// 输入，匹配成功的特征点对Inliers标记
							...
							vP3D,				/// todo 特征点对经过三角测量之后的空间坐标，也就是地图点 这里有地图点生成
 ...
 * 
 * @param[in & out] vbMatchesInliers          标记是否是内点
                                 ...
                                 cv::Mat &H21)
{
    ...
    //! todo Step 1 将当前帧和参考帧中的特征点坐标进行归一化，主要是平移和尺度变换
    /// 具体来说,就是将mvKeys1和mvKey2归一化到均值为0，一阶绝对矩为1，归一化矩阵分别为T1、T2
    /// todo 这里所谓的一阶绝对矩其实就是随机变量(特征点)到取值的中心的绝对值的平均值
    ...
    {
        /// Select a minimum set
		//! Step 2 选择8个归一化之后的点对进行迭代  读取8对特征点的归一化之后的坐标
        for(size_t j=0; j<8; j++)
        {
...
cv::Mat Initializer::ComputeH21(
    const vector<cv::Point2f> &vP1, /// 归一化后的点, in reference frame vPn1i
    const vector<cv::Point2f> &vP2) /// 归一化后的点, in current frame vPn2i
{
    // 基本原理：见附件推导过程：
    // |x'|     | h1 h2 h3 ||x|
	...
	/// step 1 获取参与计算的特征点的数目
    const int N = vP1.size();

    /// step 2 构造用于计算的矩阵 A
    cv::Mat A(2*N,				//行，注意每一个点的数据对应两行 一对点提供两个约束？  N = 4
			  9,				//列
			  CV_32F);      	//float数据类型

	/// step 3 构造矩阵A，将每个特征点添加到矩阵A中的元素    N = 4？
    for(int i=0; i<N; i++)
    {
		//获取特征点对的像素坐标
	...
	///  step 4 使用opencv提供的进行奇异值分解的函数
    cv::SVDecomp(A,							//输入，待进行奇异值分解的矩阵
				 w,							    //输出，奇异值矩阵
				 u,							    //输出，矩阵U
				 vt,						    //输出，矩阵V^T
                  cv::SVD::MODIFY_A | 		//输入，MODIFY_A是指 允许计算函数可以修改待分解的矩阵，官方文档上说这样可以加快计算速度、节省内存
    ...
    float sigma)                        /// 估计误差
{
    ...
    // 算法目标： 检查单应变换矩阵
    // todo 检查方式：通过H矩阵，进行参考帧和当前帧之间的双向投影，并计算起加权最小二乘投影误差
 ...
 * @param[in] vbMatchesInliers          匹配好的特征点对 (的Inliers标记)
                               ...
                               )
{
    ...
    ///todo vbMatchesInliers 中存储匹配点对是否是有效
    int N=0;
    ...
    //!todo 原理：若某一组合使恢复得到的3D点位于相机正前方的数量最多，那么该组合就是最佳组合
 ...
 * @param[in] minTriangulated           为了进行运动恢复，所需要的最少的三角化测量成功的点个数 50
                               ...
                               )
{
    ...
    /**
     *
    目的 ：通过单应矩阵H恢复两帧图像之间的旋转矩阵R 和平移向量 t
    ...
    /// SVD分解正常情况下特征值di应该是正的，且满足d1>=d2>=d3 如果不符合 返回
    ...
    for(size_t i=0; i<8; i++)
    {
        ...
        int nGood = CheckRT(vR[i],vt[i],                    ///TODO:当前组解的旋转矩阵和平移向量 CheckRT 重要
                            mvKeys1,mvKeys2,                ///特征点
        ...
        /// 如果当前组的good计数小于历史最优但却大于历史次优  就更新次优的解
    ...
    cv::Mat &x3D)               /// 三维点
{
	...
	//!根据前面的结论，奇异值分解右奇异矩阵的最后一行其实就是解，原理类似于前面的求最小二乘解，四个未知数四个方程正好正定
                            ...
                            cv::Mat &T)                           //将特征点归一化的矩阵
{
    ...
    // 求出平均到每个点上，其坐标偏离横纵坐标均值的程度；将其倒数作为一个尺度缩放因子 得出一阶绝对矩
    meanDevX = meanDevX/N;
    meanDevY = meanDevY/N;
    ...
    /// Step 3 将x坐标和y坐标分别进行尺度归一化，使得x坐标和y坐标的一阶绝对矩分别为1
    /// 这里所谓的一阶绝对矩其实就是 随机变量到取值的中心的绝对值的平均值（期望）
    for(int i=0; i<N; i++)
    {
    ...
    /// Step 4 计算归一化矩阵：其实就是前面做的操作用矩阵变换来表示而已  为了后面变换回去方便
    /// |sX  0  -meanx*sX|
    /// |0   sY -meany*sY|
    /// |0   0      1    |
...
/**
 * @brief 用恢复出来的位姿来对特征匹配点三角化，从中筛选中合格的三维点
 * 
 * @param[in] R                                     旋转矩阵R
 * @param[in] t                                     平移矩阵t
...
{   
	...
	/// 最终结果是K*[R|t]
    P2 = K*P2;
    //!第二个相机的光心在世界坐标系下（第一个坐标系下 T21）的坐标
    cv::Mat O2 = -R.t()*t;
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        /// todo 利用三角法恢复三维点p3dC1 单目系统中第一次恢复3D地图点
        ...
        /// todo isFinite() 函数用于检查其参数是否是无穷大。
        /// todo 如果 number 是 NaN（非数字），或者是正、负无穷大的数，则返回 false。
		...
		/// todo: dot()返回的是两个数组的点积(dot product)
        float cosParallax = normal1.dot(normal2)/(dist1*dist2);

        /// Check depth in front of first camera (only if enough parallax, as "infinite" points can easily go to negative depth)
        /// 如果深度值为负值，为非法三维点跳过该匹配点对
        /// todo ?视差比较小时，重投影误差比较大。这里0.99998 对应的角度为0.36°,这里不应该是 cosParallax>0.99998 吗？ 确实是bug
        ...
        ///todo 将空间点p3dC1变换到第2个相机坐标系下变为p3dC2
        cv::Mat p3dC2 = R*p3dC1+t;	
		/// 判断过程和上面的相同
        if(p3dC2.at<float>(2)<=0 && cosParallax<0.99998)
            continue;

        //! todo Step 5 第三关：计算空间点在参考帧和当前帧上的重投影误差，如果大于阈值则舍弃
        ...
        //  todo Step 6 统计经过检验的3D点个数，记录3D点视差角
        /// 如果运行到这里就说明当前遍历的这个特征点对靠谱，经过了重重检验，说明是一个合格的点，称之为good点  把夹角余弦值保存  3D坐标保存
