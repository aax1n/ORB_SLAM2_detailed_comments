 ...
 *  
 */
///初始化列表方式赋值  （）里为传进来的参数 .clone（）是cv::mat形式存储的
///cv::mat 相比于Eigen 对动态矩阵更高效
Frame::Frame(const cv::Mat &imLeft,
    ...
    mK(K.clone()),
    mDistCoef(distCoef.clone()), ///相机去畸变参数
    mbf(bf),
    ...
    mpReferenceKF(static_cast<KeyFrame*>(NULL))     ///参考关键帧
{
	...
	///获取每层图像的缩放因子
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();  ///GetScaleFactors() 比GetScaleFactor()多了个s 1.2^m 次方吧
	///同样获取每层图像缩放因子的倒数
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();///Inverse
	///高斯模糊的时候，使用的方差
	...
	///如果左图像中没有成功提取到特征点那么就返回，也意味着这一帧的图像无法使用
    if(mvKeys.empty())
        return;
	...
	/// mvuRight中存储的是左图像中的点所匹配的在右图像中的点的横坐标（纵坐标相同）
...
Frame::Frame(const cv::Mat &imGray,
             ...
             const float &thDepth)
    :mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)  ///:初始化列表操作
{
    /// Frame ID
	//! Step 1 帧的ID 自增
    ...
    /// ORB extraction
	//! todo Step 3 对这个单目图像进行提取特征点, 第一个参数0-左图， 1-右图   ExtractORB() 很重要函数
    ExtractORB(0,imGray);

	///求出特征点的个数
    N = mvKeys.size();  ///size()返回向量中元素的个数

	///如果没有能够成功提取出特征点，那么就直接返回了
    if(mvKeys.empty())
        return;

    //! todo Step 4 用OpenCV的矫正函数、内参对提取到的特征点进行矫正
    UndistortKeyPoints();
...
void Frame::AssignFeaturesToGrid()
{
	...
	/// ? 这里0.5 是为什么？节省空间？ 答：又不可能所有特征点都平均分配到网格
    ...
    for(int i=0;i<N;i++)
    {
		...
		/// 存储某个特征点所在网格的**网格坐标**，nGridPosX范围：[0,FRAME_GRID_COLS], nGridPosY范围：[0,FRAME_GRID_ROWS]
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    if(flag==0)
        ...
        /**
         * 比如重载+ 实现自定义类型加法运算 （通过成员函数重载 +） Person P3 = P1.operator+(P2)  可以简化为Person P3 =p1 + p2
         * 也可以通过全局函数重载运算符 更普通一些
         *
         */
...
{
    /// mbTrackInView是决定一个地图点是否进行***重投影***的标志
    ...
    /// Step 2 关卡一：将这个地图点变换到当前帧的相机坐标系下，如果深度值为正才能继续下一步。
    if(PcZ<0.0f)  /// todo 相机后面还发红（有点俗）的那些点 给他去掉
        return false;
    ...
    ///  mOw：当前相机光心在世界坐标系下坐标
    const cv::Mat PO = P-mOw;  ///当前地图点距离相机光心距离（向量）
	///取模就得到了距离
    ...
    if(viewCos<viewingCosLimit)  ///余弦值越小 角度越大
        return false;
                                        ...
                                        const int maxLevel) const
{
    ...
    /// 查找半径为r的圆左侧边界所在网格列坐标。这个地方有点绕，慢慢理解下： (对应图很好理解)
    /// (mnMaxX-mnMinX)/FRAME_GRID_COLS：表示列方向每个网格可以平均分得几个像素（肯定大于1） eg: 640/30
    ...
    /// 保证nMinCellX 结果大于等于0
    const int nMinCellX = max(0,(int)floor( (x-mnMinX-r)*mfGridElementWidthInv)); ///floor 向下取整


	/// 如果最终求得的圆的左边界所在的网格列超过了设定了上限，那么就说明计算出错，找不到符合要求的特征点，返回空vector
    if(nMinCellX>=FRAME_GRID_COLS)  ///FRAME_GRID_COLS 图像分策划那个多少个网格 这里作者定义的是64
        return vIndices;

	/// 计算圆所在的右边界网格列索引
    ///从第零行开始索引 所以是FRAME_GRID_COLS-1
    const int nMaxCellX = min((int)FRAME_GRID_COLS-1, (int)ceil((x-mnMinX+r)*mfGridElementWidthInv));
	/// 如果计算出的圆右边界所在的网格不合法，说明该特征点不好，直接返回空vector
    if(nMaxCellX<0)      /// <0 好离谱
        return vIndices;
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            ...
            /// 前面已经把特征点分配到了不同的网格里面了
            /// todo mGrid[ix][iy]这个向量中存储的是每个图像网格内特征点的id
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                if(bCheckLevels)
                {
                    ...
                    if(kpUn.octave<minLevel)   ///比最小的小
                        continue;
                    if(maxLevel>=0)		///? 为何特意又强调？感觉多此一举 因为上面判断是或的关系啊
                        if(kpUn.octave>maxLevel)   ///比最大的还大
                            continue;
                }               

                /// 通过检查，计算候选特征点到圆中心的距离，查看是否是在这个圆形区域之内
                /// todo 补充：一开始范围是个长方形 通过 |x,y| < r 过滤出来就是在以r为半径的圆形区域内了
                const float distx = kpUn.pt.x-x;   ///出现负值呢
                const float disty = kpUn.pt.y-y;

				/// 如果x方向和y方向的距离都在指定的半径之内，存储其index为候选特征点
                ///fabs() cmath库函数  fabs(double x); 参数 x 是求绝对值的数。 返回值：参数 x 的绝对值
    ...
    return vIndices;
}  /// 为什么要搞这么复杂   因为使用网格搜索快啊  网格为空的话直接跳过了
...
void Frame::ComputeBoW()
{
    ...
    ///DBoW2::BowVector mBowVec;  内部实际存储的是std::map<WordId, WordValue>
    /// WordId 和 WordValue 表示Word在叶子中的id 和权重
    if(mBowVec.empty())
    {
		/// 将描述子mDescriptors转换为DBOW要求的输入格式
        vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
		/// 将特征点的描述子转换成词袋向量mBowVec以及特征向量mFeatVec    transform()第三方库函数
...
void Frame::UndistortKeyPoints()
{
    ...
    if(mDistCoef.at<float>(0)==0.0)    ///cv::mat 类型
    {
        mvKeysUn=mvKeys;
        return;
    }
    ...
    /// Step 2 如果畸变参数k1不为0，用OpenCV函数进行畸变矫正
    /// Fill matrix with points
    /// N为提取的特征点数量，为满足OpenCV函数输入要求，将N个特征点保存在N*2的矩阵中
    cv::Mat mat(N,2,CV_32F);   ///特征点坐标（u，v）
	///遍历每个特征点，并将它们的坐标保存到矩阵中
    for(int i=0; i<N; i++)
    {
		///然后将这个特征点的横纵坐标分别保存
        mat.at<float>(i,0)=mvKeys[i].pt.x;  ///更新矩阵内值 这里没有实例化出来一个新容器
    ...
    {
		///根据索引获取这个特征点
		///注意之所以这样做而不是直接重新声明一个特征点对象的目的是，能够得到原特征点对象的其他属性
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)	
{
    ...
    if(mDistCoef.at<float>(0)!=0.0)
	{
        ...
        mat.at<float>(0,0)=0.0;         ///左上 x
		mat.at<float>(0,1)=0.0;         ///左上 y
...
void Frame::ComputeStereoMatches()
{
    ...
    /// 为匹配结果预先分配内存，？？这里只是实例化容器吧. 数据类型为float型
    /// mvuRight存储右图匹配点索引 叫左图特征点对应在右图中匹配点的索引（存的是右图中的点）
    /// mvDepth存储特征点的深度信息
	mvuRight = vector<float>(N,-1.0f);   ///N 特征点个数 初值为-1
    mvDepth = vector<float>(N,-1.0f);

	/// orb特征相似度阈值  -> mean ～= (max  + min) / 2  设置为平均值
    const int thOrbDist = (ORBmatcher::TH_HIGH+ORBmatcher::TH_LOW)/2;  ///数字运算

    /// 金字塔底层（0层）图像高（行数） nRows  比如640
    const int nRows = mpORBextractorLeft->mvImagePyramid[0].rows;

	/// 二维vector存储每一行的orb特征点的列坐标的**索引**，为什么是（二维）vector，因为每一行的特征点有可能不一样，例如
    /// vRowIndices[0] = [1，2，5，8, 11]   第1行有5个特征点,他们的列号（即x坐标）分别是1,2,5,8,11
    /// vRowIndices[1] = [2，6，7，9, 13, 17, 20]  第2行有7个特征点.etc
    vector<vector<size_t> > vRowIndices(nRows, vector<size_t>()); ///每一行是vector<size_t>()
    for(int i=0; i<nRows; i++) vRowIndices[i].reserve(200);            ///每行（i 一行一行的）分配200个特征点内存

	/// 右图特征点数量，N表示数量 r表示右图，且不能被修改
    const int Nr = mvKeysRight.size();

	/// Step 1. （右图）行特征点统计。 考虑用图像金字塔尺度作为偏移，左图中对应右图的一个特征点可能存在于多行，而非唯一的一行
    for(int iR = 0; iR < Nr; iR++) {
        ...
        /// 计算特征点iR在行方向上（上下方向上），可能的偏移范围r，即可能的行号为[kpY + r, kpY -r]
        /// 2 表示在全尺寸(scale = 1)的情况下，假设有2个像素的偏移，随着尺度变化，r也跟着变化
        const float r = 2.0f * mvScaleFactors[mvKeysRight[iR].octave];///2*某层上的尺度因子
        const int maxr = ceil(kpY + r);
        const int minr = floor(kpY - r);

        /// 将特征点iR保证在可能的行号中
        for(int yi=minr;yi<=maxr;yi++)
            vRowIndices[yi].push_back(iR);  ///范围内的特征点全都push进去
    }

    /// 下面是 粗匹配 + 精匹配的过程
    /// 对于立体矫正后（极线匹配后）的两张图，在列方向(x)存在最大视差maxd和最小视差mind
    ...
    for(int iL=0; iL<N; iL++) {
        ...
        const int &levelL = kpL.octave; ///第i个特征点所在金字塔层数
        const float &vL = kpL.pt.y;    ///像素坐标
        const float &uL = kpL.pt.x;

        /// 获取左图特征点il所在行，以及在右图对应行中可能的匹配点
        const vector<size_t> &vCandidates = vRowIndices[vL]; ///对应在右图中最好匹配点可能存在的行   这里是粗匹配过程
        if(vCandidates.empty()) continue;  ///跳过这个特征点 继续！

        /// 计算理论上的最佳搜索范围   这里都是横坐标
        ...
        size_t bestIdxR = 0;   ///右图中对好的匹配点索引
        const cv::Mat &dL = mDescriptors.row(iL); /// 做图特征点描述子
        
        /// Step 2. 粗配准。左图特征点il与右图中的可能的匹配点进行逐个比较,得到最相似匹配点的描述子距离和索引
        for(size_t iC=0; iC<vCandidates.size(); iC++) {

            const size_t iR = vCandidates[iC]; ///iR只是索引 下一步才是解出描述子
            const cv::KeyPoint &kpR = mvKeysRight[iR];

            /// 左图特征点il与待匹配点ic的空间尺度差超过2，放弃
            if(kpR.octave<levelL-1 || kpR.octave>levelL+1)  ///要匹配的话 特征点所在金字塔层数不能相差太大
                continue;
            ...
            if(uR >= minU && uR <= maxU) {
                ...
                const cv::Mat &dR = mDescriptorsRight.row(iR);  ///右图对应特征点描述子
                const int dist = ORBmatcher::DescriptorDistance(dL,dR);  ///***计算描述子距离（理解为相似度更好）******

				///统计最小相似度及其对应的列坐标(x)  全都更新一下
                if( dist<bestDist ) {
                    bestDist = dist;
                    bestIdxR = iR;
                }
        ...
        if(bestDist<thOrbDist) {
            ...
            const float scaleFactor = mvInvScaleFactors[kpL.octave]; ///等于左图特征点深度，对应金字塔层数应该也可以共用左图的
            
           /// 尺度缩放后的左右图特征点坐标  在金字塔某曾图像上的坐标
            ...
            IL.convertTo(IL,CV_32F);  ///转换为32位浮点型
            ...
            for(int incR=-L; incR<=+L; incR++) {
                ...
                /// sad 计算，值越小越相似
                float dist = cv::norm(IL,IR,cv::NORM_L1);  ///用L1范数计算距离
                ...
                vDists[L+incR] = dist; 	 ///因为incR初始为-L
            }
            ...
            */
            ...
            const float dist1 = vDists[L+bestincR-1];	///最好点左边那个点
            const float dist2 = vDists[L+bestincR];     ///最好点
            const float dist3 = vDists[L+bestincR+1];   ///最好点右边那个点
            ...
            /// 根据亚像素精度偏移量delta调整最佳匹配索引  uR=   看看uR定义  应该是投影点距离光心的横坐标
            float bestuR = mvScaleFactors[kpL.octave]*((float)scaleduR0+(float)bestincR+deltaR); ///三个相加 更精确的匹配点索引
    ...
    const float median = vDistIdx[vDistIdx.size()/2].first; ///取出中值  因为中值不受离群点影响
    const float thDist = 1.5f*1.4f*median;

    for(int i=vDistIdx.size()-1;i>=0;i--) {
        if(vDistIdx[i].first<thDist)  ///说明合法 不需要删东西
            break;
        else {
