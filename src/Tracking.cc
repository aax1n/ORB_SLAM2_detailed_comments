        ...
        mnLastRelocFrameId(0)                               ///恢复为0,没有进行这个过程的时候的默认值
{
    ...
    DistCoef.copyTo(mDistCoef);  ///畸变矩阵 copy 给 mDistCoef

    /// mbf = 双目摄像头baseline * fx = 50
    ...
    /// TODO：tracking过程都会用到mpORBextractorLeft作为特征点提取器  读进来一张图像开始提取ORB特征
    ...
    )
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) ///没有成功初始化的前一个状态就是NO_IMAGES_YET
        ...
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
            mpORBextractorLeft,     ///正常运行时的ORB特征点提取器，提取指定数目特征点
            mpORBVocabulary,
            mK,
            mDistCoef,
            mbf,
...
void Tracking::Track()
{
    ...
    else  ///初始化成功了
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            /// todo Step 2：跟踪进入正常SLAM模式，有地图更新
            /// 是否正常跟踪
            if(mState==OK)
            {
        ...
        else   ///进入纯定位模式
        {
            ...
            /// todo Step 3：只进行跟踪tracking，局部地图不工作
            if(mState==LOST)
            {
            ...
            else    
            {
                ...
                if(!mbVO)
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOK = TrackWithMotionModel();   ///有速度就用恒速模型跟踪
                ...
                else ///进入 mbVO为 true
                {
                    ...
                    else if(bOKReloc) ///不再比较bOKMM 之关心重定位结果 更相信重定位（因为重定位很复杂啊）
                    {
        ...
        /// todo Step 3：在跟踪得到当前帧初始姿态后，现在对local map进行跟踪得到更多的匹配，并优化当前位姿
        ///todo 前面只是跟踪一帧得到初始位姿，这里搜索局部关键帧、局部地图点，和当前帧进行投影匹配，得到更多匹配的MapPoints后进行Pose优化
        if(!mbOnlyTracking)
        {
            if(bOK)
                bOK = TrackLocalMap();
        ...
        else   ///仅仅跟踪
        {
            ...
            /// mbVO true表示mappoint在map中没有匹配。我们不能检索
            ///局部映射，因此我们不执行TrackLocalMap()。一旦系统重新本地化
            ///相机我们将再次使用本地地图。
       ...
       ///todo Step 4：更新显示线程中的图像、特征点、地图点等信息
        mpFrameDrawer->Update(this);

        /// If tracking were good, check if we insert a keyframe
        ///todo 只有在成功追踪时才考虑生成关键帧的问题
        if(bOK)  /// 如果跟踪成功
        {
            /// Update motion model
            /// todo Step 5：跟踪成功，更新恒速运动模型
            ...
            /// todo Step 7：清除恒速模型跟踪中 UpdateLastFrame中为当前帧临时添加的MapPoints（仅双目和rgbd）
            /// 步骤6中只是在当前帧中将这些MapPoints剔除，这里从MapPoints数据库中删除
            /// 临时地图点仅仅是为了提高双目或rgbd摄像头的帧间跟踪效果，用完以后就扔了，没有添加到地图中
            for(list<MapPoint*>::iterator lit = mlpTemporalPoints.begin(), lend =  mlpTemporalPoints.end(); lit!=lend; lit++)
            {
                MapPoint* pMP = *lit;
                delete pMP;  ///删除指针  （最好指针置空 防止野指针出现）
            }

            /// 这里不仅仅是清除mlpTemporalPoints，通过delete pMP还删除了指向的MapPoint指针
            /// 不能够直接执行这个是因为其中存储的都是指针,之前的操作都是为了避免内存泄露
            mlpTemporalPoints.clear();

            /// Check if we need to insert a new keyframe
            /// todo Step 8：检测并插入关键帧，对于双目或RGB-D会产生新的地图点
            if(NeedNewKeyFrame())
                CreateNewKeyFrame();
            ...
            /// todo Step 9 删除那些在bundle adjustment中检测为outlier的地图点
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                /// 这里第一个条件还要执行判断是因为, 前面的操作中可能删除了其中的地图点  重复删除可能报错
                /// 是地图点并且是外点
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
            }
        ...
        /// todo Step 10 如果初始化后不久就跟踪失败，并且relocation也没有搞定，只能重新Reset
        if(mState==LOST)
        {
    ...
    /// todo Step 11：记录位姿信息，用于最后保存所有的轨迹
    if(!mCurrentFrame.mTcw.empty())
    {
        ...
        mlFrameTimes.push_back(mCurrentFrame.mTimeStamp); ///记录时间戳
        mlbLost.push_back(mState==LOST);   ///记录状态
    }
    else
    {
...
/**
 ...
 todo 局部关键帧和局部关键点构成了局部地图
 */
void Tracking::MonocularInitialization()
{
    //! Step 1 如果单目初始器还没有被创建，则创建。后面如果重新初始化时会清掉这个
    /// 由参考帧创建初始化器，如果没有初始化器，说明参考帧都没有，则创建参考帧
    if(!mpInitializer)
    {
        /// Set Reference Frame
        /// 单目初始帧的特征点数必须大于100
        ...
        {
            /// 初始化需要两帧，分别是mInitialFrame，mCurrentFrame  当前帧就是参考帧 第一帧
            mInitialFrame = Frame(mCurrentFrame);
            /// 用当前帧更新上一帧
            mLastFrame = Frame(mCurrentFrame);
            /// mvbPrevMatched  记录"上一帧"所有特征点坐标
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());
            for(size_t i=0; i<mCurrentFrame.mvKeysUn.size(); i++)
                mvbPrevMatched[i]=mCurrentFrame.mvKeysUn[i].pt; ///mvbPrevMatched 记录特征点去畸变后的坐标
    ...
    else    ///如果单目初始化器已经被创建 进来的当前帧就是第二帧了  第一帧已经成为了初始化帧
    {
        /// Try to initialize
        //! Step 2 如果当前帧特征点数太少（不超过100），则重新构造初始器
        /// todo NOTICE 只有连续两帧的特征点个数都大于100时，才能继续进行初始化过程
        if((int)mCurrentFrame.mvKeys.size()<=100)
        {
            delete mpInitializer;           ///删除上一帧已经构造好的初始化器
        ...
        //! todo Step 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
        ORBmatcher matcher(
            0.9,          ///todo 最佳的和次佳特征点评分的比值阈值，这里是比较宽松的，跟踪时是0.7
            true);      ///todo 检查特征点的方向
        ...
        vector<bool> vbTriangulated; /// Triangulated Correspondences (mvIniMatches) 记录特征点是否被成功三角化

        //!todo Step 5 通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints(下一步)
        /// Initialize 里面一千多行代码
            ...
            vbTriangulated))            ///以及对应于mvIniMatches来讲,其中哪些点被三角化了
        {
            ...
            mInitialFrame.SetPose(cv::Mat::eye(4,4,CV_32F));  //! eye: R = I   t = 0;  初始帧
            /// 由Rcw和tcw构造Tcw,并赋值给mTcw，mTcw为世界坐标系到相机坐标系的变换矩阵
            cv::Mat Tcw = cv::Mat::eye(4,4,CV_32F);
            Rcw.copyTo(Tcw.rowRange(0,3).colRange(0,3));
            tcw.copyTo(Tcw.rowRange(0,3).col(3));
            mCurrentFrame.SetPose(Tcw);   /// 当前帧

            //! Step 8 创建初始化地图点MapPoints
            /// Initialize函数会得到mvIniP3D，
            /// mvIniP3D是cv::Point3f类型的一个容器，是个存放3D点的临时变量，
            //! todo CreateInitialMapMonocular将3D点包装成MapPoint类型存入KeyFrame和Map中
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    //!todo Step 1 将初始关键帧,当前关键帧的描述子转为BoW
    pKFini->ComputeBoW();
    pKFcur->ComputeBoW();

    // Insert KFs in the map
    //! todo Step 2 将关键帧插入到地图
    mpMap->AddKeyFrame(pKFini);
    mpMap->AddKeyFrame(pKFcur);

    // Create MapPoints and asscoiate to keyframes
    //! todo Step 3 用初始化得到的3D点来生成地图点MapPoints
    //!  mvIniMatches[i] 表示初始化两帧特征点匹配关系。  vector重载了[] 或者.at(i)
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        //!todo  Step 3.1 用3D点构造MapPoint
        MapPoint* pMP = new MapPoint(
            worldPos,
            pKFcur, 
            mpMap);

        //! todo Step 3.2 为该MapPoint添加属性：
        /// a.观测到该MapPoint的关键帧
        /// b.该MapPoint的描述子
        /// c.该MapPoint的平均观测方向和深度范围
        ...
        /// todo b.从众多观测到该MapPoint的特征点中挑选最有代表性的描述子
        pMP->ComputeDistinctiveDescriptors();
        /// todo c.更新该MapPoint平均观测方向以及观测距离的范围
        ...
        ///Add to Map  mpMap 是整个地图 pMP是地图点
    ...
    //! **************Bundle Adjustment***********
    /// mpMap->MapPointsInMap() 获得当前地图中的地图点个数
    cout << "New Map created with " << mpMap->MapPointsInMap() << " points" << endl;

    //! todo Step 4 全局BA优化，同时优化所有位姿和三维点
    Optimizer::GlobalBundleAdjustemnt(mpMap,20);

    // Set median depth to 1
    //! todo Step 5 *********************取场景的中值深度，用于尺度归一化************************8
    ...
    mvpLocalKeyFrames.push_back(pKFini);
    /// todo 单目初始化之后，得到的初始地图中的所有点都是局部地图点
    mvpLocalMapPoints=mpMap->GetAllMapPoints();
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    vector<MapPoint*> vpMapPointMatches; ///三维点 地图点的匹配对

    /// Step 2：通过词袋BoW加速当前帧与参考帧之间的特征点匹配  SearchByBoW()很重要
    ...
    /// Discard outliers
    /// Step 5：剔除优化后的匹配点中的外点
    /// 之所以在优化之后才剔除外点，是因为在优化的过程中就(不是才？)有了对这些外点的标记
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
...
void Tracking::UpdateLastFrame()
{
    ...
    /// 上一普通帧的参考关键帧，注意这里用的是参考关键帧（位姿准）而不是上一帧的普通帧
    KeyFrame* pRef = mLastFrame.mpReferenceKF;    /// public:  KeyFrame* mpReferenceKF  Frame下成员变量 KeyFrame*类型
    /// ref_keyframe 到 lastframe的位姿变换
    cv::Mat Tlr = mlRelativeFramePoses.back();   /// Tracking类成员 list<cv::Mat> mlRelativeFramePoses;  Mat  存储矩阵类型  back() 最后一个变换矩阵

    /// 将上一帧的世界坐标系下的位姿计算出来
    /// l:last, r:reference, w:world
    /// Tlw = Tlr*Trw
    mLastFrame.SetPose(Tlr*pRef->GetPose());  //void Frame::SetPose(cv::Mat Tcw) Frame类下成员函数  cv::Mat KeyFrame::GetPose() Keyframe类下成员函数（对外接口）

    //! 如果上一帧为关键帧，或者单目的情况，则退出(上一关键帧ID == 关键帧ID 或者为单目 return掉)
    ...
    /// We sort points according to their measured depth by the stereo/RGB-D sensor  根据深度把特征点排序
    //! Step 2.1：得到上一帧中具有有效深度值的特征点（不一定是地图点）
    vector<pair<float,int> > vDepthIdx;    /// pair对组 两个类型 相当于key—value
    vDepthIdx.reserve(mLastFrame.N);   /// reserve(int len)  vector预留空间 减少vector在动态扩展容量时的扩展次数  预留位置不初始化 元素不可访问 只是分配了内存
                                          /// N:  Number of KeyPoints.
    for(int i=0; i<mLastFrame.N;i++)      /// 遍历上一帧所有特征点
    {
        float z = mLastFrame.mvDepth[i];  ///    std::vector<float> mvDepth; Frame类下成员变量  vector重载了[] 利用[]或at访问值
        if(z>0)
        {
            /// vDepthIdx第一个元素是某个点的深度,第二个元素是对应的特征点id
            vDepthIdx.push_back(make_pair(z,i)); ///make_pair(value1, value2) 创建对组  深度，索引存入  上面定义时用了另一种创建方式
        }
    }
    ...
    /// 按照深度从小到大排序
    sort(vDepthIdx.begin(),vDepthIdx.end());  ///默认从小到大排序  需要从大到小排序 需要写个仿函数传进去 或者用内置函数对象（greater）

    /// We insert all close points (depth<mThDepth)
    /// If less than 100 close points, we insert the 100 closest ones.
    //! Step 2.2：从中找出是三维点不是地图点的部分
    int nPoints = 0;
    for(size_t j=0; j<vDepthIdx.size();j++)  ///size_t是一种机器相关的无符号类型，它被设计的足够大以便能表示内存中任意对象的大小
    {                                        ///在64位中是unsigned long int，在32位中是unsigned int
        int i = vDepthIdx[j].second;   ///second 为pair对组的第二个元素 i 有深度特征点的id

        bool bCreateNew = false;

        /// 如果这个点对应在上一帧中的地图点没有,或者创建后就没有被相机观测到,那么就生成一个临时的地图点
        ///pMP 第一次被创建    Frame类下  std::vector<MapPoint*> mvpMapPoints;  []访问值
        MapPoint* pMP = mLastFrame.mvpMapPoints[i];
        if(!pMP)
            bCreateNew = true;  ///标记为true
        ...
        if(bCreateNew)
        {
            ...
            /// 反投影到世界坐标系中
            /// cv::Mat Frame::UnprojectStereo(const int &i)   return cv::Mat(); ///
            ...
            /// 标记为临时添加的MapPoint(因为只负责局部跟踪用)，之后在CreateNewKeyFrame之前会全部删除
...
bool Tracking::TrackWithMotionModel()
{
    ...
    /// Step 2：根据之前估计的速度？（ 确定不是上一帧位姿？），用恒速模型得到当前帧的初始位姿。
    /// mVelocity = Tcl = Tcw * Twl,表示上一帧到当前帧的变换， 其中 Twl = LastTwc
    mCurrentFrame.SetPose(mVelocity*mLastFrame.mTcw);  ///上一帧到当前帧变化乘上一帧位姿 ===当前帧位姿
...
bool Tracking::TrackLocalMap()
{
    ...
    ///我们有一个相机姿态的估计和一些地图点在帧中跟踪。
    ///我们检索局部映射并尝试找到与局部映射中的点匹配的点。

    /// Update Local KeyFrames and Local Points
    /// todo Step 1：更新局部关键帧 mvpLocalKeyFrames 和局部地图点 mvpLocalMapPoints
    UpdateLocalMap();

    ///todo Step 2：筛选局部地图中新增的在视野范围内的地图点，投影到当前帧搜索匹配，得到更多的匹配关系
    SearchLocalPoints();
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])  ///是否地图点存在
        {
    ...
    /// Decide if the tracking was succesful         判断跟踪是否成功
    /// More restrictive if there was a relocalization recently   如果最近有一个重新本地化

    /// Step 5：根据跟踪匹配数目及重定位情况决定是否跟踪成功
    /// 如果最近刚刚发生了重定位,那么至少成功匹配 50个点才认为是成功跟踪
    if(mCurrentFrame.mnId<mnLastRelocFrameId+mMaxFrames && mnMatchesInliers<50)
        return false;

    ///如果是正常的状态话（最近没有发生重定位）只要跟踪的地图点大于30个就认为成功了
 ...
 * 
 * Step 1：将当前帧构造成关键帧 （普通帧升级-->关键帧）
 * Step 2：将当前关键帧设置为当前帧的参考关键帧 （关键帧升级-->参考关键帧）
 * Step 3：对于双目或rgbd摄像头，为当前帧生成新的MapPoints
 */
void Tracking::CreateNewKeyFrame()
{
    /// 如果局部建图线程关闭了,就无法插入关键帧     LocalMapping* mpLocalMapper;
    if(!mpLocalMapper->SetNotStop(true))
        return;

    ///局部线程没有关闭 Step 1：将当前帧构造成关键帧  pKF 第一次构建
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB); //KeyFrame 构造函数
    /// Step 2：将当前关键帧设置为当前帧的参考关键帧
    /// 在UpdateLocalKeyFrames函数中会将与当前关键帧共视程度最高的关键帧设定为当前帧的参考关键帧
    mpReferenceKF = pKF;  ///重载了 =  运算符
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                {
                    bCreateNew = true;
                    mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL); /// static_cast是C++的一种转换运算符，用于强制隐式转换
                }
                ...
                if(bCreateNew)
                {
                    ...
                    pNewMP->AddObservation(pKF,i);          ///给地图点添加观测
                    pKF->AddMapPoint(pNewMP,i);        ///Add MapPoint to KeyFrame; pMP MapPoint;  idx MapPoint在KeyFrame中的索引
                    pNewMP->ComputeDistinctiveDescriptors();   ///计算地图点最具代表性的描述子
                    pNewMP->UpdateNormalAndDepth();            ///更新地图点的平均观测方向、观测距离范围
                    mpMap->AddMapPoint(pNewMP);           ///向地图中插入地图点
 ...
 * 注意：局部地图点中已经是当前帧地图点的不需要再投影，只需要将此外的(新添加进来的地图点)并且在视野范围内的点和当前帧进行投影匹配
 */
void Tracking::SearchLocalPoints()
{
    /// Do not search map points already matched
    ///前面参考关键帧定位，恒速模型，重定位 三种模式进行了初步的跟踪 有一部分地图点已经被匹配过了
    /// todo Step 1：遍历**当前(原有的？)**帧的地图点，标记这些地图点-----目的是不参与之后的投影搜索匹配
    ...
    ///将局部地图点投影到当前帧 生成新的匹配
    ///todo Step 2：判断所有局部地图点中除当前帧地图点外的点，是否在当前帧视野范围内
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        /// 判断地图点是否在在当前帧视野内
        if(mCurrentFrame.isInFrustum(pMP,0.5)) ///0.5 余弦值阈值
        {
        	/// 观测到该点的帧数加1
            pMP->IncreaseVisible();
            /// 只有在视野范围内的地图点才参与之后的投影匹配
            nToMatch++;  ///主要是进行这一步 记录有多少个需要进行匹配点地图点
        }
    }
    ...
    if(nToMatch>0)
    {
        ...
        if(mCurrentFrame.mnId<mnLastRelocFrameId+2) ///如果当前帧比之前重定位后两帧时间都没过  就要扩大搜索范围
            th=5;
...
void Tracking::UpdateLocalMap()
{
    ...
    /// mvpLocalMapPoints ---> vpMPs（所有地图点：黑色） ---> mvpReferenceMapPoints
    /// --->vpRefMPs(通过MapDrawer.cc GetReferenceMapPoints()函数) --->spRefMPs(红色绘制)
    mpMap->SetReferenceMapPoints(mvpLocalMapPoints);
...
void Tracking::UpdateLocalPoints()
{
    ...
    ///todo Step 2：遍历局部关键帧 mvpLocalKeyFrames
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        KeyFrame* pKF = *itKF;
        const vector<MapPoint*> vpMPs = pKF->GetMapPointMatches(); ///获取该关键帧跟踪好的的地图点

        ///todo step 2.1：将局部关键帧的地图点添加到mvpLocalMapPoints
        for(vector<MapPoint*>::const_iterator itMP=vpMPs.begin(), itEndMP=vpMPs.end(); itMP!=itEndMP; itMP++)
        {
            MapPoint* pMP = *itMP; ///每个局部关键帧的局部地图点都给他取出来
            if(!pMP)
                continue;
            /// 用该地图点的成员变量mnTrackReferenceForFrame 记录当前帧的id
            /// 表示它已经是当前帧的局部地图点了，可以防止重复添加局部地图点
            /// （因为这个地图点可能被多个关键帧观测到 肯定不能重复添加啊  或者说本帧添加过就不要添加了 给他一个标记）
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)  ///mCurrentFrame.N 当前帧中的所有特征点  N  int类型
    {
        if(mCurrentFrame.mvpMapPoints[i]) ///特征点对应的地图点
        {
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
            if(!pMP->isBad())
            {
                /// 得到观测到该地图点的关键帧和该地图点在关键帧中的索引  map 健值对
                const map<KeyFrame*,size_t> observations = pMP->GetObservations();
                /// 由于一个地图点可以被多个关键帧观测到,因此对于每一次观测,都对观测到这个地图点的关键帧进行累计投票
                for(map<KeyFrame*,size_t>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
                    /// 这里的操作非常精彩！
                    /// map[key] = value，当要插入的键存在时，会覆盖键对应的原来的值。（因为map中不允许存相同的数据，只有一份）如果键不存在，则添加一组键值对
                    /// it->first 是地图点看到(被观测到)的关键帧，同一个关键帧看到的地图点会累加到该关键帧计数
                    /// 所以最后keyframeCounter 第一个参数表示某个(有共视关系的)关键帧，第2个参数表示该关键帧看到了多少当前帧(mCurrentFrame)的地图点，也就是共视程度
                    keyframeCounter[it->first]++;  /// 一级共视关系
            } else
            {
    ...
    /// 没有任何一个关键帧观测到当前帧这些地图点 返回
    if(keyframeCounter.empty())
        return;

    /// 存储具有最多观测次数（max）的关键帧   （因为当前帧会有很多地图点  上面是每个地图点去遍历找能观测到的关键帧  所以会有某一关键帧观测次数这一概念）
    int max=0;
    KeyFrame* pKFmax= static_cast<KeyFrame*>(NULL);

    ///TODO Step 2：更新局部关键帧（mvpLocalKeyFrames），添加局部关键帧有3种类型
    /// 先清空局部关键帧
    mvpLocalKeyFrames.clear();
    /// 先申请3倍内存，不够后面再加  vector是动态数组 先分配估计可以提高运行速度
    mvpLocalKeyFrames.reserve(3*keyframeCounter.size());

    /// All keyframes that observe a map point are included in the local map. Also check which keyframe shares most points
    /// 有道：所有观察地图点的关键帧都包含在局部地图中。还要检查哪个关键帧共享最多的点
    ///TODO Step 2.1 类型1：能观测到当前帧地图点的关键帧作为局部关键帧 （将邻居拉拢入伙）（一级共视关键帧）
    for(map<KeyFrame*,int>::const_iterator it=keyframeCounter.begin(), itEnd=keyframeCounter.end(); it!=itEnd; it++)
    {
        KeyFrame* pKF = it->first; /// 得到一级共视关键帧
        ...
        pKF->mnTrackReferenceForFrame = mCurrentFrame.mnId;   ///mnId帧的ID
    }


    /// Include also some not-already-included keyframes that are neighbors to already-included keyframes
    ///还要记录一些尚未包含的关键帧，它们是已经包含的关键帧的邻居
    /// TODO Step 2.2 遍历一级共视关键帧，寻找更多的局部关键帧
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        KeyFrame* pKF = *itKF;  ///上面的一级共视关键帧给他解出来

        /// 类型2:一级共视关键帧的共视（前10个）关键帧，称为二级共视关键帧（将邻居的邻居拉拢入伙）
        /// 如果共视帧不足10帧,那么就返回所有具有共视关系的关键帧（不足10个 就全遍历呗 下面的操作是只要共视程度最高的那一帧）
        const vector<KeyFrame*> vNeighs = pKF->GetBestCovisibilityKeyFrames(10);
        /// vNeighs 是按照共视程度从大到小排列
        for(vector<KeyFrame*>::const_iterator itNeighKF=vNeighs.begin(), itEndNeighKF=vNeighs.end(); itNeighKF!=itEndNeighKF; itNeighKF++)
        {
            KeyFrame* pNeighKF = *itNeighKF; ///邻居帧解出来
            if(!pNeighKF->isBad())
            {
                ...
                if(pNeighKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
                {
                    ...
                    pNeighKF->mnTrackReferenceForFrame=mCurrentFrame.mnId; ///记录一下 防止下面重复添加
                    ///? 找到一个就直接跳出for循环？ 是的，不用怀疑！
                    break;
                }
            }
        }
        ...
        const set<KeyFrame*> spChilds = pKF->GetChilds(); ///  pKF 存的都是一步步添进来的局部关键帧
        for(set<KeyFrame*>::const_iterator sit=spChilds.begin(), send=spChilds.end(); sit!=send; sit++)
        {
            KeyFrame* pChildKF = *sit; ///远方亲戚帧解出来
            if(!pChildKF->isBad())
            {
                if(pChildKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId) ///判断这帧是否被记录过 没有就添加进来
                {
        ...
        KeyFrame* pParent = pKF->GetParent(); ///  pKF 存的都是一步步添进来的局部关键帧
        if(pParent)
        {
            /// mnTrackReferenceForFrame防止重复添加局部关键帧
            if(pParent->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
            {
                ...
                //! 感觉是个bug！如果找到父关键帧会直接跳出整个循环  作者会bug三次？
                break;
            }
        }
