...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames()) ///如果闭环队列中有关键帧
        {
...
{
    {
        ...
        ///mlpLoopKeyFrameQueue存储了参与到回环检测的关键帧
    ...
    /// 2、检测“子候选组”中每一个关键帧是否存在于“连续组”，如果存在 nCurrentConsistency++，
    ///    则将该“子候选组”放入“当前连续组vCurrentConsistentGroups”
    ...
    /// 连续性(Consistency):称之为连续长度可能更合适,表示累计的连续的链的长度:A--B 为1, A--B--C--D 为3等;
    /// 具体反映在数据类型 ConsistentGroup.second上
    /// 连续组(Consistent group): mvConsistentGroups存储了上次执行回环检测时, 新的被检测出来的具有连续性的多个组的集合.
    /// 由于组之间的连续关系是个网状结构,因此可能存在一个组因为和不同的连续组链都具有连续关系,而被添加两次的情况(当然连续性度量是不相同的)
    /// 连续组链:自造的称呼,类似于菊花链A--B--C--D这样形成了一条连续组链.对于这个例子中,由于可能E,F都和D有连续关系,因此连续组链会产生分叉;
    /// 为了简化计算,连续组中将只会保存最后形成连续关系的连续组们(见下面的连续组的更新)
    /// 子连续组: 上面的连续组中的一个组
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            for(set<KeyFrame*>::iterator sit=spCandidateGroup.begin(), send=spCandidateGroup.end(); sit!=send;sit++)
            {
                ...
                {
                    /// 如果候选组中某个关键帧在子连续组存在，该“子候选组”与该“子连续组”相连
                    bConsistent=true;
...
bool LoopClosing::ComputeSim3()
{
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        /// Step 1.2 将当前帧 mpCurrentKF 与闭环候选关键帧pKF匹配vvpMapPointMatches
        /// 通过bow加速得到 mpCurrentKF 与 pKF 之间的匹配特征点
        /// vvpMapPointMatches 是匹配特征点对应的地图点,本质上来自于候选闭环关键帧
        /// vvpMapPointMatches存放匹配的结果
        ...
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            if(!Scm.empty())
            {
                ...
                if(nInliers>=20)
                {
                    ...
                    /// mvpCurrentMatchedPoints 记录匹配关系
...
/**
 * @brief 闭环矫正
 * 1. 通过求解的Sim3以及相对姿态关系，调整与当前帧相连的关键帧位姿  以及 调整这些关键帧观测到的地图点位置（相连关键帧---当前帧）
 * 2. 将闭环帧以及闭环帧相连的关键帧的地图点 和 与当前帧相连的关键帧的点进行匹配（当前帧+相连关键帧---闭环帧+相连关键帧）
...
void LoopClosing::CorrectLoop()
{
    ...
    /// g2oCorrectedSiw：世界坐标系到当前关键帧的共视关键帧的Sim3 变换
    ...
    /// KeyFrameAndPose map类型  key----keyframe   value----Sim3
    ...
    /// 对地图点操作
    {
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            /// 其实是现在已经有了更新后的关键帧组中关键帧的位姿,但是在上面的操作时只是暂时存储到了 KeyFrameAndPose 类型的变量中,
            /// 还没有写回到关键帧对象中
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        /// Step 3 遍历闭环帧组的所有的地图点，替换掉（当前组）需要替换的地图点
        for(int i=0; i<nLP;i++)
        {
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    /// 所以和更新后的地图并不连续。需要通过spanning tree来传播
    {
        ...
        /// 而如果被强行中断的线程退出时新的线程还没有启动,那么上面的条件就不起作用了
        ...
        if(!mbStopGBA)
        {
            ...
            for(size_t i=0; i<vpMPs.size(); i++)
            {
                ...
                /// 如果这个地图点直接参与到了全局BA优化的过程,那么就直接重新设置位姿即可
