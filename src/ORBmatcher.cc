...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        /// Step 3 通过 投影点 以及 搜索窗口 和 预测的尺度 -->进行搜索, 找出搜索半径内的候选匹配点索引
        const vector<size_t> vIndices =
                F.GetFeaturesInArea(pMP->mTrackProjX,pMP->mTrackProjY,    /// 该地图点投影到一帧上的坐标 u , v
                                    r*F.mvScaleFactors[nPredictedLevel],    /// 认为搜索窗口的大小和该特征点被追踪到时所处的尺度也有关系
                                    nPredictedLevel-1,nPredictedLevel);     /// 搜索的图层范围

        /// todo 没找到候选的,就放弃对当前地图点的匹配
        if(vIndices.empty())
            continue;
        /// todo 搜索到就取出当前地图点的描述子
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::Mat &d = F.mDescriptors.row(idx); ///取出当前帧特征点描述子
            ...
            if(dist<bestDist)
            {
                bestDist2=bestDist; /// 最好的给次好
                bestDist=dist;     /// 现在的给最好
                bestLevel2 = bestLevel;  /// 最好的层级给次好的
                bestLevel = F.mvKeysUn[idx].octave; /// 现在层级给最好的
                bestIdx=idx; ///更新最好的索引
            }
        ...
        /// Step 5 筛选最佳匹配点
        /// 最佳匹配距离还需要满足在设定阈值内
        /// static const int ORBmatcher::TH_HIGH = 100
        /// 要用到的一些阈值
        /// const int ORBmatcher::TH_HIGH = 100;
        /// const int ORBmatcher::TH_LOW = 50;
        /// const int ORBmatcher::HISTO_LENGTH = 30;
        if(bestDist<=TH_HIGH)
        {
            ...
            F.mvpMapPoints[bestIdx]=pMP; ///匹配上了 并记录一下索引
            nmatches++;  /// 匹配的点+1
        }
    }
    ...
    while(KFit != KFend && Fit != Fend)
    {
        ...
        if(KFit->first == Fit->first) 
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF); /// 取出KF中该特征点对应的描述子 取描述子好像都是.row（）
                ...
                if(bestDist1<=TH_LOW) 
                {
                    ...
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2)) ///比例是0.7  初始化时是0.9
                    {
                        /// Step 4.3：记录成功匹配特征点的对应的地图点(来自关键帧)
                        vpMapPointMatches[bestIdxF]=pMP; /// 地图点

                        /// 这里的realIdxKF是当前遍历到的关键帧的特征点id
                        const cv::KeyPoint &kp = pKF->mvKeysUn[realIdxKF]; /// 特征点
                                        ...
                                        int windowSize)
{
    ...
    ///[2,5,8,9,11]  F1 中的第1个特征点（idx）和F2中的第2个特征点匹配上了 初始化为-1说明名没有对应的匹配（没有匹配关系）
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        ///GetFeaturesInArea() 返回vIndices--候选点
        /// todo 通过网格搜索特征点
        ...
        /// Step 3 遍历搜索搜索（圆形）窗口中的所有潜在的匹配候选点，找到最优的和次优的
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            size_t i2 = *vit;  /// 把候选特征点索引解出来
            /// 取出候选特征点对应的描述子
        ...
        if(bestDist<=TH_LOW)   ///第一关
        {
            /// 最佳距离比次佳距离要小于设定的比例，这样特征点辨识度更高
            ///float mfNNratio --- 最优评分和次优评分的比例 -- 0.9
            if(bestDist<(float)bestDist2*mfNNratio) ///第二关
            {
                /// 如果找到的候选特征点对应F1中特征点已经匹配过了，说明发生了重复匹配，***将原来的匹配也删掉***
                if(vnMatches21[bestIdx2]>=0)  ///第三关
                {
                    vnMatches12[vnMatches21[bestIdx2]]=-1;  /// 把该匹配值设置为-1 [] 访问的是值 ===把匹配好的F1中特征点索引的值删掉
                    nmatches--;
                }
                /// 次优的匹配关系，双向建立
                /// vnMatches12保存参考帧F1和F2匹配关系，index保存是F1对应特征点索引，值保存的是匹配好的F2特征点索引
                vnMatches12[i1]=bestIdx2; /// []访问的是值 代表修改F2的特征点索引 完美对应
                vnMatches21[bestIdx2]=i1;
                vMatchedDistance[bestIdx2]=bestDist;
                nmatches++;   ///匹配点+1

                /// Step 5 计算匹配点**旋转角度差**所在的直方图
                ...
                if(mbCheckOrientation)
                {
                    ...
                    /// 如果bin 满了又是一个轮回 就放到第一个bin里
                    if(bin==HISTO_LENGTH)
                        bin=0;
                    assert(bin>=0 && bin<HISTO_LENGTH); ///assert宏的原型定义在<assert.h>中，其作用是如果它的条件返回错误，则终止程序执行
    ...
    for(size_t i1=0, iend1=vnMatches12.size(); i1<iend1; i1++)
        if(vnMatches12[i1]>=0)
            vbPrevMatched[i1]=F2.mvKeysUn[vnMatches12[i1]].pt; ///vbPrevMatched[i1]保存的是当前帧（F2）对应的匹配点

    return nmatches;  ///返回匹配好的特征点个数
}
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1,
                                       KeyFrame *pKF2,
                                       cv::Mat F12,
                                       vector<pair<size_t, size_t> > &vMatchedPairs,  ///对组
                                       const bool bOnlyStereo)
{
    ...
    const float ex =pKF2->fx*C2.at<float>(0)*invz+pKF2->cx; ///归一化后的x  极点在归一化平面？
    const float ey =pKF2->fy*C2.at<float>(1)*invz+pKF2->cy; ///归一化后的y
    ...
    /// f1it->first对应node编号，f1it->second对应属于该node的所有特特征点编号（容器）
    DBoW2::FeatureVector::const_iterator f1it = vFeatVec1.begin();
    DBoW2::FeatureVector::const_iterator f2it = vFeatVec2.begin();
    DBoW2::FeatureVector::const_iterator f1end = vFeatVec1.end();
    DBoW2::FeatureVector::const_iterator f2end = vFeatVec2.end();
        ...
        if(f1it->first == f2it->first)
        {
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    if(dist>TH_LOW || dist>bestDist)  ///距离越小越好
                        continue;
                   ...
                   /// 通过特征点索引idx2在pKF2中取出对应的特征点
                    ...
                    if(!bStereo1 && !bStereo2)
                    {
                        ...
                        ///? 对这个阈值的有效性持怀疑态度
                        if(distex*distex+distey*distey<100*pKF2->mvScaleFactors[kp2.octave])  ///两个相机比较近 三角话是不准的
                            continue;
                    }
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        /// Step 2 得到地图点投影到关键帧的图像坐标   这里是归一化坐标
        const float invz = 1/p3Dc.at<float>(2);
        const float x = p3Dc.at<float>(0)*invz;
        const float y = p3Dc.at<float>(1)*invz;
        /// 像素坐标
        ...
        const float ur = u-bf*invz;  ///双目的那个...

        const float maxDistance = pMP->GetMaxDistanceInvariance();  ///Invariance --- 不变性
        const float minDistance = pMP->GetMinDistanceInvariance();
        cv::Mat PO = p3Dw-Ow;
        ...
        /// Viewing angle must be less than 60 deg
        /// Step 4 地图点到光心的连线与该地图点的平均观测向量之间夹角要小于60°  （平均观测方向 < 60°）
        cv::Mat Pn = pMP->GetNormal();
        if(PO.dot(Pn)<0.5*dist3D)
            continue;
        /// 根据地图点到相机光心距离预测匹配点所在的金字塔尺度 (预测所在金字塔层级)
        ...
        const vector<size_t> vIndices = pKF->GetFeaturesInArea(u,v,radius);  ///在邻居关键帧中查找
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                /// 如果最佳匹配点没有对应地图点，添加观测信息 （上面融合 这里新增）
                pMP->AddObservation(pKF,bestIdx);
                pKF->AddMapPoint(pMP,bestIdx);  /// pMP 是邻居中地图点  一开始定义的
            }
            nFused++;  ///记录一下融合的地图点点
        }
    }
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            if(pMPinKF)
            {
                ...
                /// todo 这里不能直接替换，原因是需要对地图点加锁后才能替换，否则可能会crash。所以先记录，在加锁后替换
                if(!pMPinKF->isBad())
                             ...
                             const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        /// Step 3.1：通过Sim变换，将pKF1的地图点投影到pKF2中的图像坐标  （注意是地图点向pKF2投影）
        ///地图点世界坐标系取出来
        cv::Mat p3Dw = pMP->GetWorldPos();
        ...
        /// Search in a radius
        /// 计算特征点搜索半径
        /// todo 所在金字塔层数越高，越模糊，不确定性越强 --> 搜索半径越大
    ...
    /// Check agreement
    /// Step 5： 一致性检查,只有在两次互相匹配中都出现才能够认为是可靠的匹配 (你能找到我 我也得能找到你  咱俩才是一对)
    int nFound = 0;
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        if(idx2>=0)
        {
            ...
            if(idx1==i1)  ///转了一圈回来是吻合的，说明两个都恩那个互相找到
            {
                /// 更新匹配的地图点
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                /// 单目：th = 7，双目：th = 15
                float radius = th*CurrentFrame.mvScaleFactors[nLastOctave]; /// 尺度（金字塔层级）越大，搜索范围越大
                ...
                if(bForward) /// 前进,则上一帧兴趣点在所在的尺度nLastOctave（5）<=nCurOctave（6）
