...
/// 关键帧的构造函数  TODO: 初始化列表操作  传进来的F赋值给了下面（F） pMap 传给了（pMap）  pKFDB 传给了（pKFDB）清晰的令人发指！
///初始化列表相当于用构造函数去赋值操作
...
void KeyFrame::UpdateConnections()
{
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            /// 所以最后KFcounter 第一个参数表示某个关键帧，第2个参数表示该关键帧可以看到） 看到了多少（当前帧的地图点，也就是共视程度
            KFcounter[mit->first]++;
        }
    }
...

    {
        ...
        /// Step 5 更新生成树的连接
        /// 更新关键帧信息：对于第一次加入生成树的关键帧，取共视程度最高的关键帧
        /// 该操作会改变当前关键帧的成员变量mpParent和父关键帧的成员变量mspChildrens
        /// mbFirstConnection是否第一次加入到生成树 默认为true
        if(mbFirstConnection && mnId!=0)    /// 并且不是第一帧
        {
            /// 初始化共视程度最高的那个关键帧为关键帧的父关键帧
...
/// todo 其实过程就是对当前关键帧下所有地图点的深度进行从小到大排序,返回距离头部其中1/q处的深度值作为当前场景的平均深度
