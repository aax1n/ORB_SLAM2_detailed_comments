 ...
 * Step 1：找出和当前帧具有公共单词的**所有关键帧**，不包括与当前帧连接的关键帧
...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            ...
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                ...
                if(pKFi->mnLoopQuery!=pKF->mnId)    
                {
                    ...
                    pKFi->mnLoopWords=0;
                    /// 和邻接关键帧共视的话不作为闭环候选帧  （黄绿点不能有共视关系（公公单词））
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        if(pKFi->mnLoopWords>minCommonWords)
        {
            ...
            pKFi->mLoopScore = si;
            if(si>=minScore)
                lScoreAndMatch.push_back(make_pair(si,pKFi)); ///得到一些候选回环关键帧
        }
    }
    ...
    {
        ///lScoreAndMatch.push_back(make_pair(si,pKFi)); it->second是候选关键帧
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
        ...
        for(DBoW2::BowVector::const_iterator vit=F->mBowVec.begin(), vend=F->mBowVec.end(); vit != vend; vit++)
        {
            ...
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                ...
                if(pKFi->mnRelocQuery!=F->mnId)
                {
                    ...
                    lKFsSharingWords.push_back(pKFi); ///同当前帧都能观测到相同单词的关键帧给push_back 到lKFsSharingWords
                }
                pKFi->mnRelocWords++; ///记录公共单词个数
            }
        }
    }
    ...
    /// Only compare against those keyframes that share enough words
    /// Step 2：统计上述关键帧中与当前帧F具有共同单词最多的单词数maxCommonWords，用来设定阈值
    int maxCommonWords=0;
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        /// 更新一下最大的公共单词数目
    ...
    list<pair<float,KeyFrame*> > lAccScoreAndMatch;  ///累加
    ...
    /// Return all those keyframes with a score higher than 0.75*bestScore
    /// Step 5：得到所有组中总得分大于阈值2的，组内得分最高的关键帧组，作为候选关键帧组
    ///阈值2：最高得分的0.75倍
    float minScoreToRetain = 0.75f*bestAccScore; 
    set<KeyFrame*> spAlreadyAddedKF;
