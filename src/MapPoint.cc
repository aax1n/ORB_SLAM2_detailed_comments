...
/**
 * ====================================
 * @brief 计算地图点最具代表性的描述子  也就是对应特征点最具代表性的描述子  一个地图点可能对应很多帧中的特征点
 *=====================================
...
{
    /// Retrieve all observed descriptors  描述子为vector类型
    vector<cv::Mat> vDescriptors;

    map<KeyFrame*,size_t> observations;

    /// Step 1 获取**该地图点**所有有效的观测关键帧信息
    {
        unique_lock<mutex> lock1(mMutexFeatures);
        if(mbBad)
            return;
        observations=mObservations; ///observations所有有效的观测关键帧信息
    }

    if(observations.empty())
        return;

    vDescriptors.reserve(observations.size()); ///reserve开辟空间 地图点有效的观测帧个数
                                                 ///（一个地图点肯定对应一个关键帧上一个特征点  所以描述子个数=有效观测关键帧数目）
    /// Step 2 遍历观测到该地图点的所有关键帧，取出对应的orb描述子，放到向量vDescriptors中  （很普通的map容器遍历）
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        /// mit->first取观测到该地图点的关键帧  keyframe
        /// mit->second取该地图点在关键帧中的索引 idx
        KeyFrame* pKF = mit->first;

        if(!pKF->isBad()) ///关键帧是好的
            /// 取对应的描述子向量
            vDescriptors.push_back(pKF->mDescriptors.row(mit->second)); ///********？？？？.row什么操作 查一下
    }
    ...
    ///float Distances[N][N];
    ///遍历for (vector<std::vector<float> >::iterator it = v.begin(); it != v.end(); it++)
    ///   {
    ///        （*it）是vector<float> 容器  所以先要遍历小容器
    ...
    ///
    ///
	std::vector<std::vector<float> > Distances; ///vector 嵌套vector 数组嵌套数组 相当于二位数组

	Distances.resize(N, vector<float>(N, 0)); /// 都分配 N个描述子的空间
	for (size_t i = 0; i<N; i++)
    {
        ...
        {
            int distij = ORBmatcher::DescriptorDistance(vDescriptors[i],vDescriptors[j]); ///就是用的对应特征点的描述子
    ...
    for(size_t i=0;i<N;i++)
    {
		...
		vector<int> vDists(Distances[i].begin(), Distances[i].end());///todo 有参构造方式进行赋值 第i个描述子开始 结尾？ 256位？
		sort(vDists.begin(), vDists.end()); ///排序 从小到大
    ...
    {
        unique_lock<mutex> lock(mMutexFeatures);
        mDescriptor = vDescriptors[BestIdx].clone();   ///最好的描述子赋给mDescriptor
    }
}
...
{
    //! Step 1 获得观测到该地图点的所有关键帧、坐标等信息  (map 容器  存放关键帧 坐标信息)
    map<KeyFrame*,size_t> observations; ///map 为key-value 形式
    KeyFrame* pRefKF;  ///参考关键帧
    cv::Mat Pos;  ///位置信息
    {
        unique_lock<mutex> lock1(mMutexFeatures); ///  std::mutex mMutexPos; 对当前地图点位姿进行操作的时候的互斥量
        unique_lock<mutex> lock2(mMutexPos);    /// std::mutex mMutexFeatures; 对当前地图点的特征信息进行操作的时候的互斥量
        if(mbBad)    ///坏点标记
            return;
       /// 观测到该MapPoint的KF和该MapPoint在KF中的索引
       /// std::map<KeyFrame*,size_t> mObservations;
        observations=mObservations; /// 获得观测到该地图点的所有关键帧（的特征点） 和 索引
    ...
    cv::Mat normal = cv::Mat::zeros(3,1,CV_32F);  /// normal 法线 朝向  列向量
    int n=0;
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        KeyFrame* pKF = mit->first;
        cv::Mat Owi = pKF->GetCameraCenter();  ///关键帧相机光心在世界坐标系坐标
        /// 获得地图点和观测到它关键帧的向量并归一化
        cv::Mat normali = mWorldPos - Owi;   ///指向被减数
        normal = normal + normali/cv::norm(normali);            ///归一化 变成单位向量 因为这是记录方向 这里是矢量相加 重载了 = + 运算符
        n++;
    } 

    cv::Mat PC = Pos - pRefKF->GetCameraCenter();                      /// 参考关键帧相机指向地图点的向量（在世界坐标系下的表示）
    const float dist = cv::norm(PC);                              /// 该点到参考关键帧相机的距离 求PC模长
...
/**
 * ===============================================
 * @brief 预测地图点对应特征点所在的图像金字塔尺度层数  m
 * ===============================================
 *
 ...
 * @return int              预测的金字塔尺度
 */
int MapPoint::PredictScale(const float &currentDist, KeyFrame* pKF) ///传入当前距离  关键帧
{
    float ratio;  ///比率
    {
        unique_lock<mutex> lock(mMutexPos);
        ...
        ratio = mfMaxDistance/currentDist; ///dmax/d = 1.2^m
    }
