...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            /// Step 4 当前关键帧与相邻关键帧重新通过三角化产生***新的****地图点，使得跟踪更加稳定   很重要
            CreateNewMapPoints();
            ...
            /// 终止BA的标志 Tracking线程中添加关键帧哪里也可以设置这个参数  要添加关键帧时，需要终止这次BA
            mbAbortBA = false;
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    {
        unique_lock<mutex> lock(mMutexNewKFs);
        /// 取出列表中最前面的关键帧，作为当前要处理的关键帧  mlNewKeyFrames 是Tracking 线程中得到的关键帧
    ...
    /// Step 3：当前处理关键帧中有效的地图点，更新normal(平均观测方向)，描述子等信息
    /// TrackLocalMap中和当前帧新匹配上的地图点和当前关键帧进行关联绑定
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();
    /// 对当前处理的这个关键帧中的所有的地图点展开遍历
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        MapPoint* pMP = vpMapPointMatches[i];  ///取出每个地图点
        if(pMP)
        {
            if(!pMP->isBad())
            {
...
///mlpRecentAddedMapPoints 是Step4 CreateNewMapPoints() 生成的，生成的地图点一方面把他放到
/// 当前地图里，另一方面把他备份存到mlpRecentAddedMapPoints里，地址是一样的，检查的时候如果地图点质量很差，给他SetBadFlag（）
/// 后对应的地图点在以后匹配什么的就会跳过他，不会使用，mlpRecentAddedMapPoints存在的意义只是对新增的做一个判断，
/// ok的就仅从mlpRecentAddedMapPoints  中删掉，地图中还有，如果不ok的在地图里还做一个bad标志，后面地图做pnp什么的就不会用这个地图点
...
void LocalMapping::MapPointCulling()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        else if(((int)nCurrentKFid - (int)pMP->mnFirstKFid) >= 2 && pMP->Observations() <= cnThObs)
        {
            ...
            /// 但是观测到该点的相机数（帧数）却不超过阈值cnThObs，从地图中删除
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
...
///==============================================================================
/**
 * @brief 用当前关键帧与相邻关键帧通过三角化产生新的地图点，使得跟踪更稳 (匹配得到新的地图点)
 * 
 */
void LocalMapping::CreateNewMapPoints()
{
    /// Retrieve neighbor keyframes in covisibility graph  在共视图中检索相邻关键帧
    /// nn表示搜索最佳共视关键帧的数目
    ...
    ///得到与该关键帧连接的前N个最强共视关键帧(已按权值排序) N:设定要取出的关键帧数目
    ///return vector<KeyFrame*>     返回的是满足权重条件的关键帧集合
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);

    /// 特征点匹配配置 最佳距离 < 0.6*次佳距离，比较苛刻了。不检查旋转
    ORBmatcher matcher(0.6,false); 

    /// 取出当前帧从世界坐标系到相机坐标系的变换矩阵 Twl
    cv::Mat Rcw1 = mpCurrentKeyFrame->GetRotation();
    cv::Mat Rwc1 = Rcw1.t(); ///t    Inverses a matrix.
    cv::Mat tcw1 = mpCurrentKeyFrame->GetTranslation();
    cv::Mat Tcw1(3,4,CV_32F);  ///3行4列  32位浮点型
    Rcw1.copyTo(Tcw1.colRange(0,3)); ///  CV::Mat内置函数 copyTo  colRang 列区间 左闭右开 [0,3) <==>0,1,2 列  R为三列嘛
    tcw1.copyTo(Tcw1.col(3)); /// 3为第四列

    /// 得到当前关键帧（左目）光心在世界坐标系中的坐标、内参   Ow1 左目光心
    ...
    const float &invfy1 = mpCurrentKeyFrame->invfy; /// invfx invfy :Calibration parameters

    /// 用于后面的点深度的验证;这里的1.5是经验值
    /// mfScaleFactor = 1.2  ratioFactor 干嘛用？
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)  ///vpNeighKFs 公式关键帧
    {
        ...
        /// Check first that baseline is not too short Check first that baseline is not too short
        /// 相邻的关键帧光心在世界坐标系中的坐标
        cv::Mat Ow2 = pKF2->GetCameraCenter();
        /// 基线向量，两个关键帧间的相机位移 这里是向量减法
        cv::Mat vBaseline = Ow2-Ow1;
        /// 基线长度
        const float baseline = cv::norm(vBaseline); /// cv::norm 计算一个矩阵的范数  这里就是求向量模长
        ...
        if(!mbMonocular)
        {
            ...
            /// 因为太短的基线下能够恢复的地图点不稳定
            if(baseline<pKF2->mb) ///mb应该是yaml文件读进来的
            continue;  ///跳出本次继续
        }
        else    
        {
            ...
            /// 基线与景深的比例
            ///两个相机（关键帧）太近了 就不作三角化了 台金三角化不太准嘛
        ...
        cv::Mat F12 = ComputeF12(mpCurrentKeyFrame,pKF2); ///传入当前关键帧 共视关键帧  返回 F 矩阵

        /// Search matches that fullfil epipolar constraint
        /// Step 5：通过词袋对两关键帧的未匹配的特征点快速匹配，用极线约束抑制离群点，生成新的匹配点对
        vector<pair<size_t,size_t> > vMatchedIndices; ///Indices 指数
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            /// ***特征点反投影,其实得到的是在各自相机坐标系下的一个非归一化的方向向（和这个点的反投影射线重合***）
            ...
            {
                /// Linear Triangulation Method   通过三角化来恢复3D点
            ...
            {
                /// 如果是双目，用视差角更大的那个双目信息来恢复，直接用已知特征点反投影了
            ...
            else if(bStereo2 && cosParallaxStereo2<cosParallaxStereo1)  
            {
                ...
                continue; ///N o stereo and very low parallax, 放弃
 ...
 * todo 自己理解： 正向过程：就是当前关键帧的地图点通过变换矩阵投影到邻居关键帧中，找到对应的特征点，在通过特征点找到对应的邻居关键帧中对应的地图点，
 * todo 如果这个地图点刚好在当前帧对应的地图点中，则不管，在自己帧中地图点集合中，就去比较观测次数进行融合，再如果没有对应的地图点则添加地图点和观测关系。
 * todo 反向过程： 邻居关键帧地图点向当前关键帧中投影 也是上面三种情况，Fuse（）函数也是同一个
 */
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
        /// 没有和当前帧进行过融合的操作
        /// pKFi->mnFuseTargetForKF == mpCurrentKeyFrame->mnId 标记有没有加入过
    ...
    vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches(); ///当前关键帧地图点取出来
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++) ///遍历相邻关键帧
    {
        KeyFrame* pKFi = *vit;
    ...
    {
        KeyFrame* pKFi = *vitKF;  ///每一邻居关键帧解出来
        vector<MapPoint*> vpMapPointsKFi = pKFi->GetMapPointMatches(); ///每一邻居帧所有地图点vector

        /// 遍历当前一级邻接和二级邻接关键帧中所有的MapPoints,找出需要进行融合的并且加入到集合中
        for(vector<MapPoint*>::iterator vitMP=vpMapPointsKFi.begin(), vendMP=vpMapPointsKFi.end(); vitMP!=vendMP; vitMP++)
        {
            MapPoint* pMP = *vitMP; ///每一个地图点解出来
            if(!pMP)
                continue;
            ...
            vpFuseCandidates.push_back(pMP);  ///加入到需要融合的候选点
        }
    }
    /// Step 4.2：进行地图点投影融合,和正向融合操作是完全相同的
    /// 不同的是正向操作是"每个关键帧和（向）当前关键帧的地图点进行融合",而这里的是"当前关键帧和（向）所有邻接关键帧的地图点进行融合"
...
void LocalMapping::KeyFrameCulling()
{
    ...
    // 该函数里变量层层深入，这里列一下：
    // mpCurrentKeyFrame：当前关键帧，todo 本程序就是判断它是否需要删除
    ...
    */
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        /// Step 3：遍历该共视关键帧的所有地图点，该地图点能被其它至少3个关键帧观测到的地图点为冗余地图点
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    if(!mbMonocular)
                    {
                        /// 对于双目或RGB-D，仅考虑近处（不超过基线的40倍 ）的地图点  不然深度值太大了 不可信  太小太大都不行
                        if(pKF->mvDepth[i]>pKF->mThDepth || pKF->mvDepth[i]<0)
                            continue;
                    }
                    /// 自增 说明这个地图点是有效的
                    nMPs++;
                    /// pMP->Observations() 是观测到该地图点的相机总数目（单目1（单目每次观测一次），双目2）
                    if(pMP->Observations()>thObs)
                    {
                        ...
                        int nObs=0;
                        /// 遍历观测到该地图点的关键帧 （要开始删除关键帧了 如过这一片地图点能被3个以上关键帧看到90%，看到这一片的关键帧就不要了）
                        for(map<KeyFrame*, size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            ...
                            /// 回答：因为同样或更低金字塔层级的地图点更准确   因为要删关键帧嘛  谨慎一些好 不要太随便删除
