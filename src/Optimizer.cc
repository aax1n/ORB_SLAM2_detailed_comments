...
/**
 * @brief 全局BA：todo ************************ pMap中所有的MapPoints和关键帧做bundle adjustment优化 *********************
                                 ...
                                 const bool bRobust)
{
    ...
    /// todo Step 2 向优化器添加顶点

    /// Set KeyFrame vertices
    /// todo Step 2.1 ：向优化器添加关键帧位姿顶点
    /// 遍历当前地图中的所有关键帧
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        vPoint->setId(id);
        /// todo 注意g2o在做BA的优化时必须将其所有地图点全部schur掉，否则会出错。
        ...
        int nEdges = 0;
        /// SET EDGES
        /// todo Step 3：向优化器添加投影边（是在遍历地图点、添加地图点的顶点的时候顺便添加的）
        /// 遍历观察到当前地图点的所有关键帧
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        else
        {
            ...
            /// 经过全局BA优化后的相机的位姿   cv::Mat mTcwGBA;
            Converter::toCvMat(SE3quat).copyTo(pKF->mTcwGBA);
            pKF->mnBAGlobalForKF = nLoopKF;
        }
    }

    /// todo Step 5.2 Points
 ...
 *         + Vertex： （节点）待优化当前帧的Tcw
 *         + measurement：MapPoint在当前帧中的二维位置(u,v)
 *         + InfoMatrix: invSigma2(与特征点所在的尺度有关)
 *
 * @param   pFrame Frame  当前帧
 * @return  inliers数量
 */
 /// todo 跟踪过程的BA
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    /// g2o::SparseOptimizer optimizer;
    /// todo 第一步 创建一个线性求解器 linearSolver
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;

    linearSolver = new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();
    /// todo 第二步 创建BlockSolver. 并用上面定义的线性求解器初始化
    g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(linearSolver);
    /// todo 第三步 创建总求解器 solver. 并从GN, LM, Dogleg 中选一个，再用上述块求解器BlockSolver初始化
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    /// todo 第四步 创建终极大boss 稀疏求解器 （SparseOptimizer）
    ...
    /// 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值5.991 ---- 对应单目
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(pFrame->mvuRight[i]<0)
            {
                ...
                obs << kpUn.pt.x, kpUn.pt.y;
                /// todo 新建单目的边，一元边，误差为观测特征点坐标减去投影点的坐标
                ...
                /// 信息矩阵  edge->setInformation ( Eigen::Matrix2d::Identity() ); // 信息矩阵2x2的单位阵
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                /// 在这里使用了鲁棒核函数
                g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                e->setRobustKernel(rk);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if(chi2>chi2Mono[it]) /// 如果卡方 > 5.991 ---- 离群点
            {                
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    /// todo Step 1 将当前关键帧及其共视关键帧加入局部关键帧 (只优化一级相邻关键帧)
    ...
    /// todo Step 3 得到能被局部地图点观测到，但不属于需要优化的局部关键帧的关键帧(二级相连)，这些二级相连关键帧在局部BA优化时不优化
    list<KeyFrame*> lFixedCameras;
    /// 遍历局部地图中的每个地图点
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        /// 观测到该地图点的KF和该地图点在KF中的索引
        ///map<KeyFrame *, size_t> MapPoint::GetObservations() 获取观测到当前地图点的关键帧
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            /// pKFi->mnBAFixedForKF!=pKF->mnId 表示还未标记为fixed（固定的）关键帧  （ 他是二级的 并且还没标记 我们给标记）
    ...
    /// Setup optimizer
    ///  todo Step 4 构造g2o优化器
    g2o::SparseOptimizer optimizer;  ///构造稀疏优化器
    ...
    /// todo Step 5 添加待优化的位姿顶点：局部关键帧的位姿
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;  ///局部关键帧解出来
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
        /// 设置初始优化位姿
        vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
        vSE3->setId(pKFi->mnId); ///ID 就是关键帧的ID
        /// 如果是初始关键帧（第零帧），要锁住位姿不优化 （根基不能优化，整个轨迹会飘了）
        vSE3->setFixed(pKFi->mnId==0);
        optimizer.addVertex(vSE3);  ///把这个关键帧pose加入到优化器顶点
    ...
    /// todo Step  6 添加不优化的位姿顶点：固定关键帧的位姿，注意这里调用了vSE3->setFixed(true)
    ///  不优化这些点  lFixedCameras.push_back(pKFi);
    /// todo 不优化为啥也要添加？回答：为了增加约束信息
    /// todo Fixedcamera是不参加优化的
    for(list<KeyFrame*>::iterator lit=lFixedCameras.begin(), lend=lFixedCameras.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setFixed(true);  
        optimizer.addVertex(vSE3);  ///添加进来但不参加优化
        if(pKFi->mnId>maxKFid)
            maxKFid=pKFi->mnId;
    }

    /// Set MapPoint vertices
    /// todo Step  7 添加待优化的局部地图点顶点 （上面添加了位姿）  两点一条边
    /// 边的最大数目 =（前二级局部关键帧） * 地图点数目
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        /// 添加顶点：MapPoint
        MapPoint* pMP = *lit;
        ///todo new一个地图点在g2o中的顶点
        g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
        /// 地图点的世界坐标系坐标设为初值
        vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));
        /// 前面记录maxKFid的作用在这里体现
        /// todo 地图点id  就是按顺序记录顶点id吧
        int id = pMP->mnId+maxKFid+1;
        vPoint->setId(id);
        /// todo 因为使用了LinearSolverType（线性求解器），所以需要将所有的三维点边缘化掉
        /// g2o中必须设置，ceres没有这个限制
        vPoint->setMarginalized(true);
        /// todo 顶点添加到优化器中
        optimizer.addVertex(vPoint);
        ...
        /// Set edges
        /// todo Step 8 在添加完了一个地图点之后, 对每一对关联的地图点和关键帧（位姿）构建边
        /// 遍历所有观测到当前地图点的关键帧
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first; ///取出每一个地图点对应的关键帧
    ...
    if(bDoMore)
    {
        ...
        /// todo Step 10 检测outlier，并设置下次不优化
        /// 遍历所有的单目误差边
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
        {
            g2o::EdgeSE3ProjectXYZ* e = vpEdgesMono[i]; ///把存的边拿出来 第一次优化后的结果
            MapPoint* pMP = vpMapPointEdgeMono[i];      ///把存的地图点拿出来 也优化了一次
        ...
        /// todo Step 11：排除误差较大的outlier后再次优化 -- 第二阶段优化
        optimizer.initializeOptimization(0);
        ///再次优化十次
    ...
    /// Check inlier observations
    /// todo Step 12：在优化后重新计算误差，剔除连接误差比较大的关键帧和地图点
    /// 对于单目误差边
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
    ...
    /// Recover optimized data
    /// todo Step 13：优化后更新 关键帧位姿 以及 地图点的位置、平均观测方向等属性
...
/// todo  ************************************ 本质图优化 **************************************************
/**
 * @brief todo 闭环检测后，EssentialGraph优化，仅优化所有关键帧位姿，不优化地图点
 *
 * 1. Vertex:
                                       ...
                                       )
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        /// Spanning tree edge
        /// todo Step 4.1：添加第2种边：生成树的边（有父关键帧）
        ...
        /// Loop edges
        /// todo Step 4.2：添加第3种边：当前帧与闭环匹配帧之间的连接关系(这里面也包括了当前遍历到的这个关键帧之前曾经存在过的回环边)
        /// 获取和当前关键帧形成闭环关系的关键帧
                            ...
                            )
{
    ...
    /// Set Sim3 vertex
    /// todo Step 2: 设置待优化的Sim3位姿作为顶点
    g2o::VertexSim3Expmap * vSim3 = new g2o::VertexSim3Expmap();    
    /// 根据传感器类型决定是否固定尺度 单目固定尺度
    vSim3->_fix_scale=bFixScale;
    ...
    vector<g2o::EdgeSim3ProjectXYZ*> vpEdges12;         /// pKF2对应的地图点到pKF1的投影边  （这里应该是误差边）
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP1 && pMP2)
        {
            if(!pMP1->isBad() && !pMP2->isBad() && i2>=0)
            {
                ...
                vPoint1->setId(id1);
                /// todo 地图点不优化 --------- 这么一大圈 结果地图点不优化 因为只优化sim3  这里地图点是准的
                /// todo 虽然是顶点 但不优化
        ...
        /// Step 4.1 闭环候选帧地图点投影到当前关键帧的边(误差) -- 正向投影
        g2o::EdgeSim3ProjectXYZ* e12 = new g2o::EdgeSim3ProjectXYZ();
        /// vertex(id2)对应的是pKF2（候选帧） VertexSBAPointXYZ 类型的三维点
        ...
        /// Set edge x2 = S21*X1
        /// todo Step 4.2 当前关键帧地图点投影到闭环候选帧的边 -- 反向投影
