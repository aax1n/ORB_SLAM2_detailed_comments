  ...
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  ...
  typename vector<TDescriptor>::const_iterator fit; /// 迭代器
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    if(!v.empty() && !must)
    {
      /// unnecessary when normalizing  归一化操作
      const double nd = v.size();
      for(BowVector::iterator vit = v.begin(); vit != v.end(); vit++) 
        vit->second /= nd;
    }
...
{ 
  ...
  vector<NodeId> nodes;
  typename vector<NodeId>::const_iterator nit; ///迭代器
  ...
  do
  {
    ...
    final_id = nodes[0];///把第一个节点id拿出来

    /// 取当前节点第一个子节点的描述子距离初始化最佳（小）距离
    double best_d = F::distance(feature, m_nodes[final_id].descriptor);  ///计算的是汉明距离
    ...
    if(nid != NULL && current_level == nid_level)  ///比如设定nidlevel==3
      *nid = final_id;
    
  } while( !m_nodes[final_id].isLeaf() );  ///不是叶子节点就继续循环
